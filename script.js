...
function renderPresets() {
  ...
  presets.forEach((p, i) => {
    ...
    document.getElementById("save-preset-btn").onclick = () => {
      const name = document.getElementById("new-preset-name").value.trim();
      if (!name || tempExercises.length === 0) {
        return alert("Anna nimi ja vähintään yksi liike!");
      }
...
function startWorkout(preset) {
  ...
  preset.exercises.forEach((ex, i) => {
    ...
    for (let s = 0; s < ex.sets; s++) {
      ...
      setDiv.innerHTML = `Setti ${s + 1}:<br> Paino <input type="number" value="0" class="weight">kg <br> Toistot <input type="number" value="0" class="reps">`;
